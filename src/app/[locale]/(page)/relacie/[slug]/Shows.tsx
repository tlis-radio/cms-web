...
function Episode({ episode, ShowName, relatedArticles, locale }: { episode: any, ShowName: string, relatedArticles?: Article[], locale: string }) {
    ...
    return (
        <div className="border bg-[#1c1c1c] p-4 text-white drop-shadow-lg">
                ...
                <div className="flex-1 flex flex-col">
                    <div className="flex justify-between items-start gap-4 md:flex-row flex-col mt-0 md:mt-2">
                        <div className="flex flex-col items-start">
                            ...
                            <p>
                                {new Date(episode.Date).toLocaleDateString(locale === 'sk' ? "sk-SK" : "en-US")} • {episode.Views} {t('views_count')}
                            </p>
                        </div>
                        ...
                        {isDescriptionOverflowing && (
                            <div className="absolute w-full bottom-0">
                                ...
                                <button
                                    ...
                                    className={"mt-2 font-bold underline absolute bottom-0 left-1/2 -translate-x-1/2 text-[#D43C4A]"}
                                >
                                    {isDescriptionExpanded ? t('hide') : t('show_more')}
                                </button>
                            </div>
                        )}
                    </div>
...
export default function Shows({ 
    ...
    currentPage: number, 
    locale: string,
    episodeArticles?: Record<string, Article[]> 
}) {
                    ...
                    <div className="border-b pb-4">
                        <div className="flex flex-col gap-6 md:flex-row">
                            ...
                            <div className="m-auto flex h-max w-full flex-col gap-4">
                                ...
                                <div className="font-argentumSansLight flex flex-wrap gap-1 justify-center w-full"> 
                                    {t('editors')}:
                                    <b className="flex flex-wrap gap-1 ml-1">
                                    {show.Cast.map((castMember: ShowCast, index: number) => (
                                        <span key={index}>
                                            <Link 
                                                href={`/${locale}/ucinkujuci/${castMember.Cast_id.Slug}`}
                                ...
                                <p className="font-argentumSansLight">{t('episodes_count')}: <b>{totalCount}</b></p>
                            </div>
                        </div>
                    </div>
