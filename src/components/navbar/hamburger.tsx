        ...
        return (
          <div key={index} className="w-full border-t max-[500px]">
            <button
              ...
              type="button"
            >
              <Link href={link.url} onClick={handleMainLinkClick} className="font-argentumSansMedium px-8 py-4 uppercase">
                {link.text}
              </Link>
            ...
            {showSubLinks && link.subLinks.map((subLink, subIndex) => (
              <div key={subIndex} className="w-full border-t bg-[#a83b38]">
                <Link href={subLink.url} target={subLink.target}>
                  <button
                    className="font-argentumSansMedium px-6 py-4 w-full text-left hover:text-[#96120F] hover:bg-white transition-colors text-center uppercase"
    ...
    <>
      ...
      <div
        className={classNames(
          "left-0 flex flex-col fixed w-full bg-[#96120F] z-20 top-[80px] transition-opacity duration-300 rounded-b-3xl lg:hidden",
          {
